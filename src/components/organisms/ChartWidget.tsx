...
export const ChartWidget: React.FC<ChartWidgetProps> = ({ title, labels, data, unit, hideTitle = false, datasets }) => {
  ...
  const startWeight = data && data.length > 0 ? data[0] : 0;
  const currentWeight = data && data.length > 0 ? data[data.length - 1] : 0;
  const totalLost = (startWeight - currentWeight).toFixed(1);
  const isLoss = Number(totalLost) >= 0;
    ...
    <View style={styles.container}>
      ...
      {(data && data.length < 2) && (!datasets || datasets[0].data.length < 2) ? (
        ...
        <LineChart
          ...
          renderDotContent={!datasets && data ? ({ x, y, index }) => {
