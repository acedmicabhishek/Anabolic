...
export const HumanBodyModel: React.FC<HumanBodyModelProps> = React.memo(({ measurements }) => {
  ...
  const renderMarker = useCallback((part: string, cx: number, cy: number, realPartName: string) => {
    const hitAreaRadius = isMale ? 50 : 20;
    ...
    return (
      <Circle 
        ...
        onPress={() => handleOpenModal(realPartName)}
      />
    );
  }, [isMale, handleOpenModal]);

  const renderBadge = useCallback((partName: string, top: DimensionValue, left: DimensionValue) => {
    const data = recentMeasurements[partName];
    if (!data) return null;
    return (
      <View key={partName} style={[styles.badgeContainer, { top, left }]}>
  ...
  }, [recentMeasurements, bodyUnit]);

  return (
    <View style={styles.wrapper}>
      <View style={styles.headerRow}>
        <Text style={styles.title}>Body Composition</Text>
        <Ionicons name="scan" size={20} color={THEME.colors.primary} style={{ opacity: 0.5 }} />
      </View>
      <View style={styles.bodyContainer}>
        {!isMale && (
          <Image 
            source={require('../../../assets/femaleoutline.png')} 
        ...
        <Svg 
          width={120} 
          height={300} 
          viewBox={isMale ? `0 0 ${MALE_VB_WIDTH} ${MALE_VB_HEIGHT}` : `0 0 ${FEMALE_VB_WIDTH} ${FEMALE_VB_HEIGHT}`}
        >
          {isMale && (
            <Path 
              d={MALE_PATH} 
              fill="rgba(141, 224, 166, 0.05)"
              stroke="rgba(141, 224, 166, 0.3)"
              strokeWidth="2"
            />
          )}
          ...
          {isMale ? (
            <>
              {renderMarker('Shoulders', 302, 350, 'Shoulders')}
              {renderMarker('Chest', 302, 550, 'Chest')}
              {renderMarker('Biceps', 120, 700, 'Biceps')}
              {renderMarker('Biceps_R', 484, 700, 'Biceps')}
              {renderMarker('Waist', 302, 850, 'Waist')}
              {renderMarker('Quads', 220, 1150, 'Quads')}
              {renderMarker('Quads_R', 384, 1150, 'Quads')}
              {renderMarker('Calves', 220, 1450, 'Calves')}
              {renderMarker('Calves_R', 384, 1450, 'Calves')}
            </>
          ) : (
            <>
              {renderMarker('Shoulders', 60, 52, 'Shoulders')}
              {renderMarker('Chest', 60, 85, 'Chest')}
              {renderMarker('Biceps', 36, 100, 'Biceps')}
              {renderMarker('Biceps_R', 84, 100, 'Biceps')}
              {renderMarker('Waist', 60, 140, 'Waist')}
              {renderMarker('Quads', 45, 200, 'Quads')}
              {renderMarker('Quads_R', 75, 200, 'Quads')}
              {renderMarker('Calves', 50, 260, 'Calves')}
              {renderMarker('Calves_R', 70, 260, 'Calves')}
            </>
          )}
        </Svg>
        
        {renderBadge('Shoulders', '10%', '75%')}
        {renderBadge('Chest', '25%', '5%')}
        {renderBadge('Biceps', '38%', '80%')}
        {renderBadge('Waist', '52%', '5%')}
        {renderBadge('Quads', '65%', '85%')}
        {renderBadge('Calves', '85%', '5%')}
      </View>
    </View>
  );
});
