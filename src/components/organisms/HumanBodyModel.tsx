...
export const HumanBodyModel: React.FC<HumanBodyModelProps> = ({ measurements }) => {
  ...
  const getMeasurementStatus = (part: string) => {
    const data = recentMeasurements[part];
    if (!data) return 'none';
    const logDate = new Date(data.date);
    const isToday = logDate.toDateString() === new Date().toDateString();
    return isToday ? 'active' : 'recorded';
  };
  ...
  const renderMarker = (part: string, cx: number, cx_val: number, realPartName: string) => {
    const isSelected = !!recentMeasurements[part];
    ...
    return (
      <Circle 
        ...
        onPress={() => openLogModal('body', realPartName as any)}
      />
    );
  };
    ...
    <View style={styles.wrapper}>
      ...
      <View style={styles.bodyContainer}>
        ...
        >
          ...
          {/* Calibrated Markers for Male SVG Scale */}
          {isMale ? (
            <>
